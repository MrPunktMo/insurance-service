...
public interface FeeApi {
    ...
    default ResponseEntity<List<FeeResponse>> getAllFees(
     ...
     * Request the calculation of an insurance fee
     *
     * @param feeRequest  (optional)
    ...
    @Operation(
        ...
        responses = {
            @ApiResponse(responseCode = "201", description = "Created", content = {
                @Content(mediaType = "application/json", schema = @Schema(implementation = FeeResponse.class))
            }),
            @ApiResponse(responseCode = "400", description = "Bad Request"),
            @ApiResponse(responseCode = "401", description = "Unauthorized"),
            @ApiResponse(responseCode = "403", description = "Forbidden"),
    ...
    default ResponseEntity<FeeResponse> postRequestFee(
        @Parameter(name = "FeeRequest", description = "") @Valid @RequestBody(required = false) FeeRequest feeRequest
    ) {
        getRequest().ifPresent(request -> {
            for (MediaType mediaType: MediaType.parseMediaTypes(request.getHeader("Accept"))) {
                if (mediaType.isCompatibleWith(MediaType.valueOf("application/json"))) {
                    String exampleString = "{ \"amount\" : 0.8008282, \"kilometrage\" : 6, \"region\" : \"region\", \"createdOn\" : \"2000-01-23\", \"vehicleType\" : \"vehicleType\" }";
                    ApiUtil.setExampleResponse(request, "application/json", exampleString);
                    break;
                }
            }
